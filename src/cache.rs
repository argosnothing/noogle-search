...
const CACHE_DIR_NAME: &str = "noogle-search-tv";
...
pub fn load_data() -> Result<NoogleResponse> {
    ...
    let response: NoogleResponse = serde_json::from_str(&data_content)
...
fn fetch_and_cache(data_path: &PathBuf, metadata_path: &PathBuf) -> Result<()> {
    let response = reqwest::blocking::get(API_URL)
        .context("Failed to fetch from Noogle API")?;
    
    let body = response.text()
        .context("Failed to read API response")?;
    
    fs::write(data_path, &body)
        .context("Failed to write data cache")?;
